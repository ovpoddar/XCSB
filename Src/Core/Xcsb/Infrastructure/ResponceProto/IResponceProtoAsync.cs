...
public interface IResponseProtoAsync
{
    ...
    Task<QueryTextExtentsReply> QueryTextExtentsAsync(uint font, ReadOnlySpan<char> stringForQuery, CancellationToken token = default);
    Task<ListFontsReply> ListFontsAsync(ReadOnlySpan<byte> pattern, int maxNames, CancellationToken token = default);
    Task<ListFontsWithInfoReply[]> ListFontsWithInfoAsync(ReadOnlySpan<byte> pattan, int maxNames, CancellationToken token = default);
    ...
    Task<AllocNamedColorReply> AllocNamedColorAsync(uint colorMap, ReadOnlySpan<byte> name, CancellationToken token = default);
    ...
    Task<QueryColorsReply> QueryColorsAsync(uint colorMap, ReadOnlySpan<uint> pixels, CancellationToken token = default);
    Task<LookupColorReply> LookupColorAsync(uint colorMap, ReadOnlySpan<byte> name, CancellationToken token = default);
    ...
    Task<SetModifierMappingReply> SetModifierMappingAsync(Span<ulong> keycodes, CancellationToken token = default);
    ...
    Task<SetPointerMappingReply> SetPointerMappingAsync(Span<byte> maps, CancellationToken token = default);
